...
# STEM DEATHS
# require(spatstat)
surv <- read.csv("oosting.survival.csv")
range(surv$diameter,na.rm=TRUE)
# all sapling stems between 2-10cm; so for taller tree deaths, can use those stems
# that are larger than 10cm DBH
...
png("map_deaths.png", width = 3, height = 3, units = "in", res = 300)
par(mar = rep(0,4))
plot(death.fun, main = "", col = rev(gray.ramp))
dev.off()

png("map_deaths_legend.png", width = 6, height = 6, units = "in", res = 300)
par(mar = rep(0,4))
plot(death.fun, main = "", col = rev(gray.ramp))
dev.off()

im <- as.im(death.fun)
ext <- im[surv[,2:3]]   # gives pixel values of death for each sapling in 'surv'
surv <- cbind(surv,ext)
